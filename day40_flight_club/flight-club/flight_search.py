...
class FlightSearch:
    ...
    def find_flights(self, orig_city, dest_city, from_time, to_time):
        ...
        try:
            ...
            # Новое
            # Попробуем с тремя пересадками
            for i in range(1, 4):
                print(f'Попробую найти с {i} пересадкой')
                try:
                    parameters['max_stopovers'] = i
                    ...
                    stop_overs=data['route'] # недописано. в падлу
                    )
                except:
                    if i < 3:
                        continue
